...
class AiVoiceBridge {
    ...
    async synthesize(params) {
        return new Promise((resolve, reject) => {
            const priority = (params.priority != null) ? params.priority : 0;
            this._synthQueue.push({ params, resolve, reject, priority, seq: ++this._synthSeq });
            this._processQueue();
        });
    }
    ...
    async _processQueue() {
        ...
        let bestIdx = 0;
        for (let i = 1; i < this._synthQueue.length; i++) {
            if (this._synthQueue[i].priority > this._synthQueue[bestIdx].priority) {
                bestIdx = i;
            }
        }
        const [{ params, resolve, reject }] = this._synthQueue.splice(bestIdx, 1);
